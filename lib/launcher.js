 ...
 * path of the registered project with the given name. The Remote Control
 * session is named after the project.
 *
...
function launchProject(name, options = {}) {
  ...
  // `--remote-control [name]` takes an optional session name. Passing the
  // project name keeps the session identifiable in the Remote Control session
  // list, instead of the auto-generated hostname-prefixed name Claude falls
  // back to when it is omitted.
  const argv = [getClaudeCommand(), getRemoteFlag(), project.name];
